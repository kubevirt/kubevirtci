...
echo "#!/bin/bash" > /usr/local/bin/ssh.sh
echo "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no vagrant@192.168.66.1${n} -i vagrant.key -p 22" >> /usr/local/bin/ssh.sh && chmod u+x /usr/local/bin/ssh.sh
