...
# Install Ansible 2.6 + Openshift 3.11 repositories
# see http://lists.openshift.redhat.com/openshift-archives/dev/2018-November/msg00005.html
yum install -y centos-release-configmanagement centos-release-paas-common
rpm -i http://mirror.centos.org/centos/7/extras/x86_64/Packages/centos-release-ansible26-1-3.el7.centos.noarch.rpm
rpm -i http://mirror.centos.org/centos/7/extras/x86_64/Packages/centos-release-openshift-origin311-1-2.el7.centos.noarch.rpm

# Ignore latest ansible from epel, openshift ansible doesn't work with version > 2.6
yum --disablerepo=epel install -y ansible
...
git clone https://github.com/openshift/openshift-ansible.git -b release-3.11 --depth 1 $openshift_ansible
