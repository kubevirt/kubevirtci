...
if [ ! -z $INSTALLER_RELEASE_IMAGE ]; then
    until  export INSTALLER_COMMIT=$(oc adm release info $INSTALLER_RELEASE_IMAGE --commits --registry-config=/etc/installer/token | grep installer | awk '{print $3}' | head -n 1); do
        sleep 1
    done
fi
...
echo pullSecret: "'$(cat  /etc/installer/token)'" >> $CLUSTER_DIR/install-config.yaml
