...
```bash
cli provision --scripts /scripts --base kubevirtci/centos:1905_01 --tag kubevirtci/k8s-1.10.11
```

Running `cli` from within docker:

```bash
docker run --privileged --rm -v ${PWD}/scripts/:/scripts/ -v /var/run/docker.sock:/var/run/docker.sock kubevirtci/cli provision --scripts /scripts --base kubevirtci/centos:1905_01 --tag kubevirtci/k8s-1.10.11
```
