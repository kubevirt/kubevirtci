...
function delete_ip_reconciler_pods() {
    set +e
    pods="$(${ksh} get pods -n default | grep 'ip-reconciler' | awk '{ print $1; }' | tr "\n" " ")"
    set -e
    [ -z "${pods}" ] && return
    set +e
    ${ksh} delete --ignore-not-found=true pods ${pods}
    set -e
}
...
(
    ...
    # whereabouts ip-reconciler cleanup: ip-reconciler cronjob pods must get removed directly after creation
    ${ksh} patch cronjob ip-reconciler --type merge -p '{ "spec": { "jobTemplate": { "spec": { "ttlSecondsAfterFinished": 0 } } } }'

    # whereabouts ip-reconciler cleanup: delete eventually lingering pods before and during the loop
    delete_ip_reconciler_pods
    until ${ksh} wait --for=condition=Ready pod --timeout=30s --all; do
        delete_ip_reconciler_pods
        sleep 1
    done
