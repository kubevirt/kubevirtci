...
# Pre pull whereabouts images and store manifests
# so we can use them at cluster-up
cp -rf /tmp/whereabouts/ /opt/
for i in $(grep --no-filename "image:" /opt/whereabouts/*.yaml | awk '{print $2}' | sort --unique); do pull_container_retry $i; done
