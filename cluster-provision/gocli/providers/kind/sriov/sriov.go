...
func NewKindSriovProvider(kindConfig *kind.KindConfig, pfCount, vfCount int) (*KindSriov, error) {
...
func (ks *KindSriov) Start(ctx context.Context, cancel context.CancelFunc) error {
	...
	case *dockercri.DockerClient:
		controlPlaneClient = docker.NewAdapter(cli, ks.Version+"-control-plane")
	...
	for _, node := range nodes {
		...
		if err = controlPlaneClient.Command("kubectl label node " + nodeName + " sriov_capable=true"); err != nil {
			return err
		}
	}
...
func (ks *KindSriov) fetchNodePfs(sshClient libssh.Client) ([]string, error) {
	mod, err := sshClient.CommandWithNoStdOut(`grep vfio_pci /proc/modules`)
	if err != nil {
		return nil, err
	}
	if mod == "" {
	...
	if err = sshClient.Command("modprobe -i vfio_pci"); err != nil {
		return nil, err
	}

	pfsString, err := sshClient.CommandWithNoStdOut(`find /sys/class/net/*/device/`)
	if err != nil {
		return nil, err
	}
...
func (ks *KindSriov) createVFsforPF(sshClient libssh.Client, vfSysClassNetPath string) ([]string, error) {
	pfSysFsDevice, err := sshClient.CommandWithNoStdOut("readlink -e " + vfSysClassNetPath)
	if err != nil {
		return nil, err
	}
	totalVfs, err := sshClient.CommandWithNoStdOut("cat " + pfSysFsDevice + "/sriov_totalvfs")
	if err != nil {
		return nil, err
	}
	...
	vfsString, err := sshClient.CommandWithNoStdOut(`readlink -e ` + pfSysFsDevice + `/virtfn*`)
	if err != nil {
		return nil, err
	}
...
func (ks *KindSriov) bindToVfio(sshClient libssh.Client, sysFsDevice string) error {
	pciAddr, err := sshClient.CommandWithNoStdOut("basename " + sysFsDevice)
	if err != nil {
		return err
	}
	...
	vfBusPciDeviceDriver, err := sshClient.CommandWithNoStdOut("readlink " + driverPath + " | awk -F'/' '{print $NF}'")
	if err != nil {
		return err
	}
	vfBusPciDeviceDriver = strings.TrimSuffix(vfBusPciDeviceDriver, "\n")
	vfDriverName, err := sshClient.CommandWithNoStdOut("basename " + vfBusPciDeviceDriver)
	if err != nil {
		return err
	}

	if _, err := sshClient.CommandWithNoStdOut("[[ '" + vfDriverName + "' != 'vfio-pci' ]] && echo " + pciAddr + " > " + driverPath + "/unbind && echo 'vfio-pci' > " + driverOverride + " && echo " + pciAddr + " > /sys/bus/pci/drivers/vfio-pci/bind"); err != nil {
		return err
	}
