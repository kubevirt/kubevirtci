...
func run(cmd *cobra.Command, args []string) (retErr error) {
	...
	for x := 0; x < int(nodes); x++ {
		...
		for i := 0; i < int(secondaryNics); i++ {
			...
			if qemuDevice == "virtio-net-ccw" {
				nodeQemuMonitorArgs = fmt.Sprintf("%s netdev_add tap,id=secondarynet%s,ifname=stap%s,script=no,downscript=no; device_add %s,netdev=secondarynet%s,mac=52:55:00:d1:56:%s;", nodeQemuMonitorArgs, netSuffix, netSuffix, qemuDevice, netSuffix, macSuffix)
...
func provisionNode(sshClient libssh.Client, n *nodesconfig.NodeLinuxConfig) error {
	...
	if n.EnableAudit {
		if err := sshClient.Command(fmt.Sprintf("touch /home/%s/enable_audit", utils.GetSSHUserByArchitecture(runtime.GOARCH))); err != nil {
