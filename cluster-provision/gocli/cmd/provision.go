...
func provisionCluster(cmd *cobra.Command, args []string) (retErr error) {
	...
	_cmd(cli, nodeContainer(prefix, nodeName), "ssh.sh sudo systemd-run --on-active=10s --unit=delayed-shutdown shutdown -h now", "shutting down the node")
	_cmd(cli, nodeContainer(prefix, nodeName), "rm /usr/local/bin/ssh.sh", "removing the ssh.sh script")
