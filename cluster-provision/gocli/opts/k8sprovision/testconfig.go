...
func AddExpectCalls(sshClient *kubevirtcimocks.MockSSHClient, version string, slim bool) {
	...
	sshClient.EXPECT().Command(`image_regex='([a-z0-9\_\.]+[/-]?)+(@sha256)?:[a-z0-9\_\.\-]+' && image_regex_w_double_quotes='"?'"${image_regex}"'"?' && grep -ioE "${image_regex_w_double_quotes}" /tmp/images`, false).Return("nginx:latest", nil)
