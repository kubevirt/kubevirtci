...
func AddExpectCalls(sshClient *kubevirtcimocks.MockSSHClient, pciID string) {
	sshClient.EXPECT().Command("lspci -D -d "+pciID, false).Return("testpciaddr something something", nil)

	devSysfsPath := "/sys/bus/pci/devices/testpciaddr"
	driverPath := devSysfsPath + "/driver"
	driverOverride := devSysfsPath + "/driver_override"

	sshClient.EXPECT().Command("readlink "+driverPath+" | awk -F'/' '{print $NF}'", false).Return("not-vfio", nil)
	sshClient.EXPECT().Command("modprobe -i vfio-pci", false)
	sshClient.EXPECT().Command("ls /sys/bus/pci/drivers/vfio-pci", false)
