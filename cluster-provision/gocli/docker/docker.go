...
func GetPrefixedContainers(cli *client.Client, prefix string) ([]types.Container, error) {
	containers, err := cli.ContainerList(context.Background(), dockercontainer.ListOptions{
		All: true,
	})
	if err != nil {
		return nil, err
			...
			case err := <-cleanupChan:
				...
				if err != nil || forceClean {
					for _, c := range createdContainers {
						if log {
							reader, err := cli.ContainerLogs(ctx, c, dockercontainer.LogsOptions{ShowStderr: true, ShowStdout: true, Details: true})
							if err == nil {
								fmt.Fprintf(os.Stderr, "\n===== %s ====\n", c)
								io.Copy(os.Stderr, reader)
							}
						}
						err := cli.ContainerRemove(ctx, c, dockercontainer.RemoveOptions{Force: true})
						if err != nil {
